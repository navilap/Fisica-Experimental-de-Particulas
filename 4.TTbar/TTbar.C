...
void TTbar()
{
    ...
    TH1F *hist_lepton_eta = new TH1F(
        "Lepton eta", "Lepton eta; eta; Events",
        50, -5, 5);

    TH1F *hist_njets = new TH1F(
        "Number of jets", "n-jets; Number of Jets; Events",
        10, 0, 10);
    ...
    TH1F *hist_nbjets = new TH1F(
        "Number of b-jets", "n-bjets; Number of b-Jet; Events",
        50, 0, 6);
    ...
    for (i = 0; i < nentries; i++)
    {
        ...
        for (unsigned int j = 0; j < lep_n; j++)
        {
            ...
            hist_lepton_pt_cone30->Fill(lep_ptcone30[j] / lep_pt[j]);
            hist_lepton_et_cone20->Fill(lep_etcone20[j] / lep_pt[j]);
            hist_lepton_eta->Fill(lep_eta[j]);
    ...
    canvas->SetLogy();
    cutflow->SetFillColor(kGreen);
    cutflow->Draw();
    canvas->Print("cutflow.pdf");
    canvas->Clear();

    hist_lepton_pt->SetFillColor(kGreen);
    hist_lep_pt->Draw();
    canvas->Print("hist_lepton_pt.pdf");
    canvas->Clear();

    hist_lepton_pt_cone30->SetFillColor(kGreen);
    hist_lepton_pt_cone30->Draw();
    canvas->Print("hist_lepton_pt_cone30.pdf");
    canvas->Clear();

    hist_lepton_et_cone20->SetFillColor(kGreen);
    hist_lepton_et_cone20->Draw();
    canvas->Print("hist_lepton_et_cone20.pdf");
    canvas->Clear();

    hist_lepton_eta->SetFillColor(kGreen);
    hist_lepton_eta->Draw();
    canvas->Print("hist_lepton_eta.pdf");
    canvas->Clear();

    hist_njets->SetFillColor(kGreen);
    hist_njets->Draw();
    canvas->Print("hist_njets.pdf");
    canvas->Clear();

    hist_jets_pt->SetFillColor(kGreen);
    hist_jets_pt->Draw();
    canvas->Print("hist_jet_pt.pdf");
    canvas->Clear();

    hist_jets_eta->SetFillColor(kGreen);
    hist_jets_eta->Draw();
    canvas->Print("hist_jet_eta.pdf");
    canvas->Clear();

    hist_jets_JVF->SetFillColor(kGreen);
    hist_jets_JVF->Draw();
    canvas->Print("hist_jet_JVF.pdf");
    canvas->Clear();

    hist_jets_MV1->SetFillColor(kGreen);
    hist_jets_MV1->Draw();
    canvas->Print("hist_jet_MV1.pdf");
    canvas->Clear();
